...
def the_connection_code(request):
    ...
    if custom_connection_url != None:
        the_connection_code = f"""from volair import VolairOnPrem
volair = VolairOnPrem('{custom_connection_url}', '{request.user.access_key}')
"""
    elif custom_connection_url == None and not use_one:
        the_connection_code = f"""from volair import VolairOnPrem
volair = VolairOnPrem('https://{request.get_host()}:7340', '{request.user.access_key}')
"""
    else:
        the_connection_code = f"""from volair import VolairOnPrem
volair = VolairOnPrem('https://{request.get_host()}/api', '{request.user.access_key}')
"""
...
def quic_start_disabled(request, exception=None):
    ...
    if not zero_scope:
        request.user.notify(
            "Congrats",
            "Your first function sent. Now you can start to use Volair",
            "info",
            section="edit"
        )
...
def control_library(request, id):
    ...
    if version != None:
        code = f'{the_name} = volair.load_module("{id}", version="{version}")'
    else:
        code = f'{the_name} = volair.load_module("{id}")'
...
def control_element(request, id):
    ...
    if version == None:
        ...
        using_code = f'''
{first_thing} = volair.load_module("{first_thing}")

{id}()
'''
    else:
        using_code = f'volair.load("{id}", version="{version}")()'
    ...
    if the_versions_response == [None]:
        ...
        for element in the_versions_response:
            ...
            data = {
                ...
                "using_code": f'volair.load("{id}", version="{element}")()',
                "link": id + ":" + element,
                "user": user,
            }
...
def control_library_version(request, id):
        ...
        the_versions.append(
            {
                ...
                "using_code": f'{the_name} = volair.load_module("{id}", version="{each_version[0]}")',
    ...
    data = {
        ...
        "no_version": no_version,
        "version": "" if version == None else version,
        "using_code": f'{the_name} = volair.load_module("{id}")',
    }
